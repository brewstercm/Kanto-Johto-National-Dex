...
return function(mod, chart, national, gen2shape, generation, era, display)
  ...
  local cutoff = tonumber(mod.exports and mod.exports.dexCutoff) or 1025
  ...
  if national then
    if type(national.text) == "table" then
      for id, value in pairs(national.text) do
        local n = type(id) == "string" and tonumber(id:match("^NDEX_(%d+)$")) or nil
        if not n or n <= cutoff then
          if safe(function()
    ...
    -- Cartridge-owned species remain native in this project.
    if type(national.register) == "table" then
      ...
      for id, record in pairs(national.register) do
        if record.form == nil and type(record.dex) == "number" and record.dex <= cutoff then
          record.spriteFront = string.format("assets/dex/front/%04d.png", record.dex)
          record.spriteBack = string.format("assets/dex/back/%04d.png", record.dex)
          record.trueColor = true
        ...
        if gen2 and gen2shape.romOwned(record) then
          -- Leave the cartridge record untouched.
        else
        ...
        end -- generation cutoff / canonical species
      end
      ...
      dexSize = math.min(dexSize, cutoff)
