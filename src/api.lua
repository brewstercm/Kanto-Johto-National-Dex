...
--
-- DISPLAY DATA. The registered `evolutions` field is the compatible gameplay
-- projection added in v0.3.13: exact levels and supported stones stay exact,
-- while friendship, location, held-item trades and other modern-only rules
-- use the documented level/stone approximations in EVOLUTION_QA.json. These
-- shards mirror those effective in-mod methods for the Dex page and API; the
-- battle engine acts only on the registered field, never on this payload.
--
