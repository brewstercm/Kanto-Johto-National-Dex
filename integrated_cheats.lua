...
return function(mod)
  ...
  mod.hooks:wrap("input.step", function(next,game,dt)
    ...
    if pendingTeleport then
      if pendingTeleportFrames>0 then
        ...
        -- Gen 2 needs the whole overlay stack removed before a warp.
  ...
  mod.content.screens:register(MAIN_SCREEN,{new=function(game)
    ...
    items[#items+1]={
      label="ALWAYS CATCH",
      right=(mod.exports.get and mod.exports.get("always_catch")) and "ON" or "OFF",
      kind="qol_always_catch"
    }
    items[#items+1]={label="USE SURFBOARD",kind="surfboard"}
    local menu
    menu=mod.ui.ListMenu.new(game,gold and "CHEATS G2" or "CHEATS G1",items,{
      pageJump=true,
      onChoose=function(item,current)
      if not item then return end
      uiPos.mainIndex=current.index or uiPos.mainIndex
      uiPos.mainScroll=current.scroll or uiPos.mainScroll
      if item.kind=="qol_always_catch" then
        if mod.exports.get and mod.exports.set then
          mod.exports.set("always_catch", not mod.exports.get("always_catch"))
        end
        current:close()
        mod.ui.push(game,MAIN_SCREEN)
        return
  ...
  mod.hooks:wrap("ui.start_menu.items",function(next,game,items)
    ...
    return mod.ui.insertBefore(out,"SAVE",{label="CHEATS",onSelect=function() mod.ui.push(game,MAIN_SCREEN) end})
  end)
end
