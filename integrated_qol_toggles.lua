...
-- chunk below: GameVersion on a real boot, else the live loader's generation (the
-- loader is Game.mods, assigned before mods run).  Older Gen 1 engines lack
-- GameVersion.generation, so the loader field remains the portable fallback.
local GEN2 = false
...
local function detectGen2()
  if GameVersion and type(GameVersion.generation) == "function" then
    ...
    if ok then return generation == 2 end
  end
...
return function(mod)
  ...
  -- Export the canonical QoL option accessors so integrated CHEATS can own
  -- cheat-class toggles without duplicating their gameplay hooks.
  mod.exports.get = get
  mod.exports.set = set
  ...
  mod.exports.toggleRows = function(getFn, setFn, gen2)
    ...
    local rows = {}
    for _, spec in ipairs(TOGGLES) do
      if spec.key ~= "always_catch" and (not spec.gen2 or gen2) then
  ...
  mod.exports.enabledCount = function(getFn, gen2)
    ...
    local n = 0
    for _, spec in ipairs(TOGGLES) do
      if spec.key ~= "always_catch" and (not spec.gen2 or gen2)
          and getFn(spec.key) then n = n + 1 end
    end
    return n
  end
  ...
  mod.exports.visibleCount = function(gen2)
    ...
    local n = 0
    for _, spec in ipairs(TOGGLES) do
      if spec.key ~= "always_catch" and (not spec.gen2 or gen2) then n = n + 1 end
    end
    return n
  end
  ...
  local LIST_VISIBLE = 4

  local function wrapFullLabel(label)
    label = tostring(label or "")
    local lines, current = {}, nil
    local maxWidth = 15 * 8
    for word in label:gmatch("%S+") do
  ...
  local function drawScrollableList(game, rows, index, scroll)
    love.graphics.setColor(1, 1, 1, 1)
    love.graphics.rectangle("fill", 0, 0, 160, 144)
    for slot = 1, LIST_VISIBLE do
      local i = scroll + slot
      local row = rows[i]
      if not row then break end
      local y = (slot - 1) * 4
      Font.drawBox(0, y, 20, 4)
      love.graphics.setColor(0, 0, 0, 1)
      local lines = wrapFullLabel(row.label)
      Font.draw(lines[1] or "", 16, (y + 1) * 8)
      if lines[2] then Font.draw(lines[2], 16, (y + 2) * 8) end
      local value = row.value and row.value(game) or ""
      local vx = math.max(16, 152 - Font.width(value))
      Font.draw(value, vx, (y + 2) * 8)
      if i == index then
        Font.drawCode(Theme.cursor, 8, (y + 1) * 8)
      end
    end
    love.graphics.setColor(0, 0, 0, 1)
    if scroll > 0 then Font.drawCode(Theme.moreArrow, 144, 0) end
    if scroll + LIST_VISIBLE < #rows then Font.drawCode(Theme.moreArrow, 144, 128) end
    Font.draw("A:TOGGLE  START:INFO  B:BACK", 8, 136)
    love.graphics.setColor(1, 1, 1, 1)
  end
  ...
  function QolTogglesMenu:update(dt)
    ...
    if self.helpRow then
      ...
      if helpRequested then helpRequested = false; close = true end
    ...
    elseif input:wasPressed("left") then
      local row = self.rows[self.index]
      if row and row.step then row.step(self.game, -1) end
    ...
    elseif get("hold_to_scroll") then
      local dir = mod.exports.holdNav(self, input, { dirs = { "up", "down" } })
      if dir == "up" then
        self.index = (self.index - 2) % #self.rows + 1
    ...
    elseif self.index > self.scroll + LIST_VISIBLE then
      self.scroll = self.index - LIST_VISIBLE
    end
    self.scroll = math.max(0, math.min(self.scroll,
      math.max(0, #self.rows - LIST_VISIBLE)))
  end

  function QolTogglesMenu:draw()
    drawScrollableList(self.game, self.rows, self.index, self.scroll)
    if self.helpRow then
