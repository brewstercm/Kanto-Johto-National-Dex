...
return function(mod)
  ...
  -- Gameplay evolutions are shipped separately from the large species file.
  -- The original payload left every added record's `evolutions` list empty,
  -- so the engine's after-battle and stone-use paths had nothing to act on.
  --
  -- Cartridge-owned records keep their native rows. Only targets introduced
  -- after that cartridge's roster are appended (for example Onix -> Steelix
  -- on R/B/Y, or Aipom -> Ambipom on Gold). Mod-owned records receive their
  -- complete generated list. Gold uses different method and target ids, so
  -- gen2shape performs the same schema translation used at registration.
  do
    local payload = loadOptionalSibling(mod,
      "data/species/generated/evolutions.lua")
    local records = payload and payload.records
    if type(records) == "table" then
      local nativeMax = generation == 2 and 251 or 151
      local applied, appended, skipped = 0, 0, 0

      local function rowsFor(entry, laterOnly)
        local source = { evolutions = {} }
        for _, evo in ipairs(entry.evolutions or {}) do
          if not laterOnly or (tonumber(evo.targetDex) or 0) > nativeMax then
            source.evolutions[#source.evolutions + 1] = {
              method = evo.method,
              level = evo.level,
              item = evo.item,
              species = evo.species,
        ...
        if generation == 2 and gen2shape then
          return gen2shape.evolutions(source)
        end
        return source.evolutions
      end

      for id, entry in pairs(records) do
        local ok, current = pcall(function()
          return mod.content.pokemon:get(id)
        end)
        if ok and type(current) == "table" then
          local native = type(entry.dex) == "number" and entry.dex <= nativeMax
          local rows = rowsFor(entry, native)
          if #rows > 0 then
            local patch = native
              and { evolutions = { __append = rows } }
              or { evolutions = rows }
            ...
            if patched then
              applied = applied + 1
              if native then appended = appended + #rows end
            else
              skipped = skipped + 1
            end
          end
      ...
      mod.log:info("gameplay evolutions applied=%d native-branches=%d "
        .. "skipped=%d", applied, appended, skipped)
    end
  end

  -- Species added beyond the running cartridge's native roster need an
  -- explicit `tmhm` list; unlike the original 151/251, there is no ROM row
  -- for the engine to consult. The payload is derived from canonical machine
  -- compatibility and restricted to that cartridge's actual TM/HM roster.
  -- Runtime move-id resolution also handles Red/Gold spelling differences
  -- and refuses to attach a reference the live move registry cannot resolve.
  do
    local payload = loadOptionalSibling(mod,
      "data/species/generated/tmhm_compatibility.lua")
    if payload then
      local Tmhm = loadSibling(mod, "src/tmhm.lua")
      if Tmhm then
        mod.exports.tmhmCompatibility = Tmhm(mod, payload, generation)
      end
    end
  end
  ...
  local widenMoves = false
  local movePayload = nil
  if movePayload then
